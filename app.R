...
txtReturnsTable <- read_file("data/returnstable.txt")
txtg1d <- read_file("data/g1d.txt")
txtcapture <- read_file("data/capture.txt")
txtboxplot <- read_file("data/boxplot.txt")
txtmktdd <- read_file("data/mktdd.txt")
txtworstdd <- read_file("data/worstdd.txt")
txtrolling <- read_file("data/rolling.txt")
txtcalyr <- read_file("data/calyr.txt")
txtscatter <- read_file("data/scatter.txt")
txtdisclaimer <- read_file("data/disclaimer.txt")

pgp <- readRDS("data/pgp_201812.rds")
...
ui <- fluidPage(title = "Red Tortoise, LLC PGP",
   
   # Application title
  fluidRow(width=12, 
           img(src='RT High.png', width = "202px", height = "56px", align = "left")),
  ...
  fluidRow(
    column(width=2,
           ...
           hr()),
    column(width=6,
           h3("PGP Hypothetical Performance"),
           hr()),
    column(width=4,
           h3("Comment"),
           hr())
   ...
   fluidRow(
      column(width=2, 
          ...
          hr(),
          sliderInput("daterange1", "Date range",
                      min = pgp$g1d$Date[1],
                      step=30,
                      max = last(pgp$g1d$Date),
                      timeFormat = "%b-%Y",
                      value = c(as.Date("1945-12-31"),last(pgp$g1d$Date))),
          helpText("Adjust slider positions to change time period.")# pgp$g1d$Date[1]
    ...
    column(width=4,
           ...
           hr(),
           h5("Disclaimer"),
           helpText(txtdisclaimer),
           hr(),
           tags$a(href="http://redtortoisellc.com/", "Red Tortoise, LLC website", target="blank")
